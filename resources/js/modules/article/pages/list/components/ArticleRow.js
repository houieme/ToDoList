  ...
  return (<tr key={index}>
    ...
    <td>
      <div className="btn-group" role="group" aria-label="Actions">
        {
          article.published
          ? <button className="btn btn-warning" onClick={() => togglePublish(article.id)}>Completée</button>
          : <button className="btn btn-success" onClick={() => togglePublish(article.id)}>Non Complotée</button>
        }
        <Link className="btn btn-primary" to={`articles/${article.id}/edit`}>Modifier</Link>
        <button className="btn btn-danger" onClick={() => handleRemove(article.id)}>Supprimer</button>
      </div>
    </td>
  </tr>)
}
